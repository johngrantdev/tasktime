  ...
  try {
    if (slugs.length > 0) {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/next/revalidate`,
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        payload.logger.info(
          `Revalidated slugs ${slugs} in collection '${collection}'`,
        );
      } else {
        const errorText = await res.text();
        payload.logger.error(
          `Error revalidating collection '${collection}': ${errorText}`,
        );
      }
    } else {
